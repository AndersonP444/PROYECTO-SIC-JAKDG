...
# Función para guardar el diccionario en formato JSON
def guardar_datos(datos):
    with open('Base de datos del BANK OF NAO.json', 'w') as f:
        json.dump(datos, f, indent=5)

# Función para cargar los datos desde el archivo JSON
def cargar_datos():
    # Verificar si el archivo JSON existe
    if os.path.exists('Base de datos del BANK OF NAO.json'):
        with open('Base de datos del BANK OF NAO.json', 'r') as f:
            return json.load(f)
    else:
        return base_de_datos_BANK_OF_NAO  # Si no existe, retornar el diccionario vacío
...
# Función principal
def main():
    datos = cargar_datos()  # Cargar los datos al inicio
...
# Cargar datos al iniciar y guardar los datos actualizados
if __name__ == "__main__":
    if not os.path.exists('Base de datos del BANK OF NAO.json'):
        guardar_datos(base_de_datos_BANK_OF_NAO)  # Guardar el diccionario vacío si no existe el archivo
    main()
